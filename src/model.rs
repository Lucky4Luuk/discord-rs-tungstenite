	...
	fn decode(kind: String, value: Value) -> Result<Event> {
		...
		if kind == "READY" {
			warn_json!(@"Event::Ready", value, Event::Ready(ReadyEvent {
				...
				read_state: try!(opt(&mut try!(remove(&mut value, "read_state")).as_object_mut().unwrap(), "entries", |v| decode_array(v, ReadState::decode))),
				...
				user_server_settings: try!(opt(&mut try!(remove(&mut value, "user_guild_settings")).as_object_mut().unwrap(), "entries", |v| decode_array(v, UserServerSettings::decode))),
