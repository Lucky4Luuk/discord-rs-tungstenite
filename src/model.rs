	...
	pub fn decode(value: Value) -> Result<LiveServer> {
		...
		warn_json!(
			...
			LiveServer {
				...
				explicit_content_filter: req!(remove(&mut value, "explicit_content_filter")?.as_u64()),
			}
		)
	}
	...
	pub fn decode(value: Value) -> Result<Option<UserSettings>> {
		...
		warn_json!(
			...
			UserSettings {
				...
				inline_attachment_media: req!(remove(&mut value, "inline_attachment_media")?.as_bool()),
				inline_embed_media: req!(remove(&mut value, "inline_embed_media")?.as_bool()),
				locale: remove(&mut value, "locale").and_then(into_string)?,
				message_display_compact: req!(remove(&mut value, "message_display_compact")?.as_bool()),
	...
	fn decode(kind: String, value: Value) -> Result<Event> {
		...
		if kind == "READY" {
			warn_json!(@"Event::Ready", value, Event::Ready(ReadyEvent {
				...
				read_state: opt(&mut remove(&mut value, "read_state")?.as_object_mut().unwrap(), "entries", |v| decode_array(v, ReadState::decode))?,
				...
				user_server_settings: opt(&mut remove(&mut value, "user_guild_settings")?.as_object_mut().unwrap(), "entries", |v| decode_array(v, UserServerSettings::decode))?,
				tutorial: opt(&mut value, "tutorial", Tutorial::decode)?,
				notes: opt(&mut value, "notes", decode_notes)?,
				// TODO: Trace doesn't seem to exist in fosscord? At least not in the READY event
			...
			warn_json!(
				value,
				Event::ServerIntegrationsUpdate(remove(&mut value, "guild_id").and_then(ServerId::decode)?)
			)
